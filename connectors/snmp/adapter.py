...
SNMP_MIN_POLL_INTERVAL_SECONDS: Final = 60
SNMP_MAX_READ_TIMEOUT_SECONDS: Final = 5
...
class SNMPv3FixtureAdapter:
    """Replay synthetic SNMPv3 envelopes with ADR-0023 contract ceilings.

    Ceiling arguments are contract dummies for this replay adapter; no live
    SNMP poll or source request is performed.
    """
    ...
    def __init__(
        ...
        if poll_interval_seconds < SNMP_MIN_POLL_INTERVAL_SECONDS:
            raise ConnectorCeilingError(
                connector="snmp",
                parameter="poll_interval_seconds",
                value=poll_interval_seconds,
                bound="at least",
                limit=SNMP_MIN_POLL_INTERVAL_SECONDS,
            )
        if read_timeout_seconds > SNMP_MAX_READ_TIMEOUT_SECONDS:
            raise ConnectorCeilingError(
                connector="snmp",
                parameter="read_timeout_seconds",
                value=read_timeout_seconds,
                bound="at most",
                limit=SNMP_MAX_READ_TIMEOUT_SECONDS,
            )
...
@final
class _SNMPv3FixtureIterator(Iterator[CanonicalEnvelope]):
    __slots__: ClassVar[tuple[str, ...]] = (
        "_enabled",
        "_fixture_paths",
        "_index",
    ...
    @override
    def __iter__(self) -> _SNMPv3FixtureIterator:
        return self
