...
Parent issue: #9
Status: closure candidate pending isolated clean-runtime run, final reviews, and
owner disposition
...
Rows classified as "to be verified by isolated clean-runtime run" are not
closure evidence until `make foundation-clean-acceptance
DCIM_RUNTIME_ROOT=<new-protected-root>` exits 0 on the final head and the
resulting public-safe summary is recorded. Ordinary `make preflight` is the
normal Development gate and may reuse selected protected state; it is not
clean-runtime proof. This matrix is therefore a closure-candidate tracker, not
an issue #9 closure artifact, until the 26 clean-runtime rows and the one
owner-disposition row are resolved with recorded evidence.

## Matrix

| # | Requirement summary | Classification | Evidence or governing source |
|---:|---|---|---|
| 1 | Explicit bootstrap command | To be verified by isolated clean-runtime run | `make foundation-clean-acceptance`; `scripts/foundation_acceptance.py`; `scripts/foundation_bootstrap.py` |
...
| 7 | PostgreSQL persistent state | To be verified by isolated clean-runtime run | clean acceptance fast/recovery evidence; `scripts/foundation_smoke.py` PostgreSQL persistence checks |
| 8 | Single-broker Kafka KRaft | To be verified by isolated clean-runtime run | clean acceptance policy/recovery evidence; Kafka environment contract in `scripts/foundation_policy.py` |
...
| 15 | Persistent volumes isolated by service and plane | To be verified by isolated clean-runtime run | acceptance namespace inventory in `scripts/foundation_acceptance.py`; policy volume checks |
| 16 | Grafana declarative provisioning with disposable state | To be verified by isolated clean-runtime run | clean acceptance Grafana smoke; Grafana tmpfs/provisioning policy checks |
| 17 | Runtime secrets generated outside Git | To be verified by isolated clean-runtime run | clean acceptance bootstrap; `scripts/protected_runtime.py`; public-safety scan |
...
| 20 | Provenance, architecture, license, SBOM, scanner, vulnerability evidence | To be verified by isolated clean-runtime run | `foundation-images-qualify`; `foundation-supply-chain`; derived-image lock and external evidence |
...
| 25 | Smoke writes refused at 90 percent usage | To be verified by isolated clean-runtime run | clean acceptance fast smoke; `capacity_disposition` and regression tests |
| 26 | Health checks for every long-running service | To be verified by isolated clean-runtime run | clean acceptance `foundation-up --wait`; health contracts in `scripts/foundation_policy.py` |
| 27 | Functional smoke separate from health status | To be verified by isolated clean-runtime run | clean acceptance fast/recovery smoke commands |
| 28 | Prometheus exposes health, scrape, Kafka, PostgreSQL, and retention signals | To be verified by isolated clean-runtime run | clean acceptance observability smoke; Prometheus config/rules |
| 29 | Grafana shows foundation health and capacity | To be verified by isolated clean-runtime run | clean acceptance Grafana health/datasource smoke; dashboard provisioning files |
...
| 31 | PostgreSQL write/read smoke | To be verified by isolated clean-runtime run | clean acceptance fast/recovery smoke evidence |
| 32 | Kafka produce/consume smoke with exact run ID | To be verified by isolated clean-runtime run | clean acceptance fast/recovery smoke evidence |
| 33 | Single allowlisted Kafka topic and 1 MiB ceiling | To be verified by isolated clean-runtime run | clean acceptance fast smoke; Kafka topic/message checks |
| 34 | Kafka-managed internal topics reported separately | To be verified by isolated clean-runtime run | clean acceptance smoke output/evidence; `kafka_topic_inventory` |
| 35 | Prometheus target, rule, and controlled-alert smoke | To be verified by isolated clean-runtime run | clean acceptance fast smoke; `controlled_alert_transition` |
| 36 | Exporter-to-backend health verified separately | To be verified by isolated clean-runtime run | clean acceptance observability smoke; `pg_up` and JMX metric checks |
| 37 | Grafana health and datasource provisioning tested | To be verified by isolated clean-runtime run | clean acceptance Grafana smoke |
| 38 | Restart recovery for PostgreSQL, Kafka, and observability | To be verified by isolated clean-runtime run | clean acceptance recovery smoke |
| 39 | PostgreSQL dump/restore with logical checksum | To be verified by isolated clean-runtime run | clean acceptance recovery smoke |
| 40 | Machine-readable smoke evidence outside Git | To be verified by isolated clean-runtime run | external `dev-build/evidence/*.json`; `scripts/foundation_evidence_summary.py` strict mode |
...
| 42 | Fast smoke on GitHub-hosted Ubuntu 24.04 runners | To be verified by isolated clean-runtime run | final PR/remote check evidence required; local clean acceptance must record OS/runtime limitations if not Ubuntu 24.04 |
...
| 44 | Bounded startup, smoke, recovery, and stop timeouts | To be verified by isolated clean-runtime run | clean acceptance step durations and exit codes; Make/Compose timeout settings |
| 45 | Bounded restart attempts | Verified by current evidence | `restart: "on-failure:3"` policy checks |
| 46 | Stop/down preserve data | To be verified by isolated clean-runtime run | clean acceptance bounded stop; existing stop/down lifecycle tests |
...
| 48 | Normalized Compose policy checks fail closed | To be verified by isolated clean-runtime run | clean acceptance policy step; `tests/test_foundation_policy.py` |
