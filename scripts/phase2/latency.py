...
from scripts.phase2 import db, noc  # noqa: E402
...
class ProducerSeam(Protocol):
    def produce_envelope(self, topic: str, key: str, value: bytes, headers: list[tuple[str, str]]) -> None: ...
    def flush(self, timeout: int) -> None: ...


class StreamSeam(Protocol):
    def capture_end_offsets(self, topic: str) -> dict[str, int]: ...
    def run_consumer(self, run_id: str, max_messages: int, idle_timeout_s: int, *, topic: str, start_offsets: Path) -> db.JsonObject: ...
...
def report(timings: Sequence[Timing], *, seed: int) -> db.JsonObject:
    ...
    persist = [int(item["persist_latency_ns"]) for item in samples]
    dashboard = [int(item["dashboard_latency_ns"]) for item in samples]
    distribution = lambda values: {"p50": nearest_rank(values, 50), "p95": nearest_rank(values, 95), "max": max(values)}
    return {
...
def assert_dashboard_latency(result: db.JsonObject) -> None:
    """Enforce the strict event/trap dashboard p95 threshold."""
    distribution = result.get("dashboard_latency_ns")
    if not isinstance(distribution, dict):
        raise LatencyError("dashboard distribution is unavailable")
    p95 = distribution.get("p95")
    if type(p95) is not int:
        raise LatencyError("dashboard p95 is invalid")
    if p95 >= P95_LIMIT_NS:
...
def _kafka_leg(run_id: str, count: int, seed: int, fixtures_dir: Path, output: Path, clock_ns: Callable[[], int]) -> list[Timing]:
    ...
    injected = list(generate(count, seed, fixtures_dir, clock_ns=clock_ns))
    for ordinal, sample in enumerate(injected):
        producer.produce_envelope(
            TOPIC,
            str(sample.envelope["event_id"]),
            json.dumps(sample.envelope, sort_keys=True, separators=(",", ":")).encode(),
            [("schema_version", "0.1.0"), ("source_run_id", run_id), ("input_ordinal", str(ordinal))],
        )
    producer.flush(30)
    stream.run_consumer(run_id, count, 30, topic=TOPIC, start_offsets=offsets_path)
    persisted = [
        (sample, _wait_until_persisted(str(sample.envelope["event_id"]), clock_ns))
...
def run_harness(*, leg: Literal["direct", "kafka"], count: int, seed: int, fixtures_dir: Path, output: Path, assert_latency: bool, run_id: str | None = None, clock_ns: Callable[[], int] = time.time_ns) -> db.JsonObject:
