...
class PublicSafetyTests(unittest.TestCase):
    def scan_text(self, name: str, text: str) -> list[object]:
    ...
    def test_safe_placeholder_and_reserved_domain_pass(self) -> None:
        key = "PASS" + "WORD"
        findings = self.scan_text(".env.example", f"{key}=<SET_LOCALLY>\nHOST=bmc-001.example.invalid\n")
        self.assertEqual([], findings)

    def test_private_ip_is_rejected(self) -> None:
        value = "10." + "23.45.67"
        findings = self.scan_text("note.md", f"endpoint={value}\n")
        self.assertTrue(any("non-documentation IP" in item.message for item in findings))
    ...
    def test_real_credential_assignment_is_rejected(self) -> None:
        ...
        self.assertTrue(any("non-placeholder password" in item.message for item in findings))

    def test_private_key_material_is_rejected(self) -> None:
        marker = "-----BEGIN " + "PRIVATE KEY-----"
        findings = self.scan_text("note.txt", marker + "\n")
        self.assertTrue(any("private key material" in item.message for item in findings))

    def test_forbidden_filename_is_rejected(self) -> None:
        findings = self.scan_text(".env", "SAFE=synthetic\n")
        self.assertTrue(any(item.rule == "forbidden-name" for item in findings))
