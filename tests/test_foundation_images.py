...
class FoundationImagesTests(unittest.TestCase):
    ...
    def test_grafana_recipe_remediates_new_kin_openapi_critical(self) -> None:
        # Given: the repository's locked Grafana derivative recipe.
        repository_manifest = json.loads(MANIFEST.read_text(encoding="utf-8"))
        grafana = next(
            item for item in repository_manifest["recipes"]
            if item["component"] == "grafana"
        ...
        # When: the Grafana recipe and its local-vendor build are inspected.
        dockerfile = (MANIFEST.parent / grafana["dockerfile"]).read_text(
            encoding="utf-8",
        )

        # Then: the vulnerable embedded module is replaced by the fixed input.
        self.assertEqual("13.1.0-r4", grafana["output_tag"])
        self.assertIn(
            {
        ...
        self.assertIn(
            "github.com/grafana/tempo google.golang.org/grpc "
            "github.com/getkin/kin-openapi",
            dockerfile,
        )
        self.assertIn(
            "grep -Fx '# github.com/getkin/kin-openapi v0.144.0' "
            "vendor/modules.txt",
            dockerfile,
        )
        self.assertIn('org.opencontainers.image.version="13.1.0-dcim.4"', dockerfile)
        self.assertIn('io.dcim.recipe.revision="4"', dockerfile)
